...
namespace AdventOfCode2021
{
    ...
    public class UnitTestDay8
    {
        ...
        [TestMethod]
        public void Day8_ShouldGiveUniqueDigits_True()
        {
            var demo = @"be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb |
...
fgae cfgab fg bagce";
            ...
            var input = FileHelper.Read(demo).ToList();
            var day = new Days8();
            day.Parse(input);
            var res = day.CountEasyDigits();
            Assert.IsTrue(res == 26);
        }

        [TestMethod]
        public void Day8_ShouldGiveUniqueDigitsResult_True()
        {
            var demo = @"fdaebc egc gc bgefc ecbagd becfa fagc afgbec gdcbfae bgdef | cgfa gcebad ebcadfg cefgbad
...
agedc cf cdeagf agcf cbefda fdcge cdgaeb bedgf dcf gbcaefd | gdefb fc egcad abecgd";
            ...
            var input = FileHelper.Read(demo).ToList();
            var day = new Days8();
            day.ParseNoLineFeed(input);
            var res = day.CountEasyDigits();
            Assert.IsTrue(res == 294);
        }
    }
}
